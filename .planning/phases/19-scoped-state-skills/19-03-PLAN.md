...
must_haves:
  ...
  key_links:
    ...
    - from: "src/parser/transformer.ts"
      ...
      via: "transform() returns StateDocumentNode for routing"
      pattern: "doc.kind === 'stateDocument'"
---
...
**Note:** parser.ts already imports from `../ir/nodes.js` (line 934 imports InputProperty types). Add the new State-related type imports to that existing import statement.
...
// Add to existing ir/nodes.js import:
...
Update the existing `import type { InputProperty, InputPropertyValue }` line near the end of parser.ts to also import StateSchema and StateSchemaField:

```typescript
import type { InputProperty, InputPropertyValue, StateSchema, StateSchemaField } from '../ir/nodes.js';
```

**Note:** `Node` is already imported from ts-morph at the top of the file (line 14).
  </action>
  <verify>
1. Run `npx tsc --noEmit` - should compile without errors
2. `grep -n "extractStateSchema" src/parser/parser.ts` shows the function
3. `grep -n "StateSchema.*from.*ir/nodes" src/parser/parser.ts` shows the import includes StateSchema
  </verify>
...
<task type="auto">
  ...
  <action>
**Pre-check:** Verify `extractTypeArguments` is already imported from parser.js. It should be on line 48:
```typescript
import { getElementName, getAttributeValue, ..., extractTypeArguments, ... } from './parser.js';
```

1. Add imports at top of transformer.ts (add to existing ir/index.js import around lines 19-47):
...
2. Add 'State' and 'Operation' to SPECIAL_COMPONENTS Set (line 67).
...
private transformState(node: JsxElement | JsxSelfClosingElement): StateDocumentNode {
  ...
  // Note: extractTypeArguments is already imported from parser.js (line 48)
...
4. Update the transform() method to handle State at root level. On **line 126**, update the return type:
...
5. Add State handling after the MCPConfig check (after line 158, inside the if block checking for JsxElement):
...
```
  ...
  <verify>
1. Run `npx tsc --noEmit` - should compile without errors
2. `grep -n "extractTypeArguments" src/parser/transformer.ts | head -5` confirms it's imported
3. `grep -n "transformState\|transformOperation" src/parser/transformer.ts` shows both methods
4. Test with a simple State TSX file to verify parsing
  </verify>
...
5. transform() method return type includes StateDocumentNode (line 126)
6. `grep -n "extractTypeArguments" src/parser/transformer.ts | head -1` confirms import exists
</verification>
