...
must_haves:
  ...
  artifacts:
    - path: "src/workflow/sections/semantic.ts"
      ...
      exports: ["OfferNext", "OfferNextRoute", "OfferNextProps", "DeviationRules", "CommitRules", "WaveExecution", "CheckpointHandling"]
...
Export `XmlWrapperProps` type for users who want to create similar wrappers.
  </action>
  <verify>TypeScript compiles: `npx tsc --noEmit src/workflow/sections/semantic.ts`</verify>
  <done>DeviationRules, CommitRules, WaveExecution, CheckpointHandling components defined with XmlWrapperProps type</done>
</task>
