...
Define interfaces with **explicit export keywords**:
```typescript
...
Export all interfaces with `export` keyword as shown above.
...
Add JSDoc comments with `@example` showing expected usage and output.
  ...
  <done>semantic.ts exists with ExecutionContext, SuccessCriteria, XmlSection exports and full TypeScript types. All interfaces and functions have explicit `export` keyword.</done>
</task>
...
<task type="auto">
  ...
  <action>
Add new IR node interfaces to `src/ir/nodes.ts` with **explicit export keywords**.
