...
must_haves:
  truths:
    ...
    - "Emitter handles SpawnAgentNode with stub that throws (preserves compilability)"
  artifacts:
    ...
    - path: "src/emitter/emitter.ts"
      provides: "Stub case for spawnAgent in emitBlock switch"
      contains: "case 'spawnAgent'"
...
7. **Add stub case in emitter.ts** to maintain exhaustiveness:
In `src/emitter/emitter.ts`, add import for `SpawnAgentNode` and add a stub case in `emitBlock()` switch (before the `default` case):
```typescript
case 'spawnAgent':
...
This keeps the project in a compilable state. The actual SpawnAgent -> Task() emission logic will be implemented in Phase 10.

Note: assertNever already exists and will provide compile-time exhaustiveness checking. The stub case ensures TypeScript is satisfied while clearly marking the work as deferred.
  </action>
  <verify>Run `npm run typecheck` - should pass with no errors. Run `npm run build` to verify compilation succeeds.</verify>
  <done>All four interfaces defined with correct discriminator kinds, BlockNode includes SpawnAgentNode, IRNode union includes all new types, emitter has stub case for spawnAgent.</done>
</task>
...
6. Emitter compiles with stub case for 'spawnAgent' (throws "not yet implemented")
</verification>
...
- Emitter has stub case for 'spawnAgent' that throws (maintains compilability)
