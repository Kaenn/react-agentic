...
must_haves:
  ...
  artifacts:
    ...
    - path: "tests/components/runtime.test.ts"
      provides: "SpawnAgent, AskUser, RuntimeCall snapshots"
      contains: "toMatchSnapshot"
...
**structured.test.ts** (Table, List from src/ir/nodes.ts):
...
**runtime.test.ts** (SpawnAgent, AskUser, RuntimeCall from src/ir/runtime-nodes.ts):
