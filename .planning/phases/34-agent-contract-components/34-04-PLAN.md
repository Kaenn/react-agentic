...
must_haves:
  truths:
    ...
    - "Agent with no contract components works (fully optional)"
    - "Status type exhaustiveness validation is tested"
  artifacts:
...
describe('Full agent with contract components', () => {
  ...
  it('works with no contract components (fully optional)', () => {
    const tsx = `
      <Agent name="bare" description="Agent without contract">
...
describe('Contract validation errors', () => {
  ...
  it('errors when Agent has status type but missing StructuredReturns', () => {
    const tsx = `
      type MyStatus = "SUCCESS" | "FAILED";

      <Agent<MyStatus> name="typed" description="Typed agent">
        <Role>I should have StructuredReturns</Role>
      </Agent>
    `;
    expectCompileError(tsx, 'must include <StructuredReturns>');
  });

  it('errors on non-exhaustive StructuredReturns', () => {
    const tsx = `
      type MyStatus = "SUCCESS" | "FAILED" | "BLOCKED";

      <Agent<MyStatus> name="typed" description="Typed agent">
        <StructuredReturns>
          <Return status="SUCCESS">All good</Return>
          <Return status="FAILED">Not good</Return>
        </StructuredReturns>
      </Agent>
    `;
    expectCompileError(tsx, 'Missing: BLOCKED');
  });
    ...
    const tsx = `
      ...
      <Agent<MyStatus> name="typed" description="Typed agent">
        <StructuredReturns>
          <Return status="SUCCESS">All good</Return>
          <Return status="FAILED">Not good</Return>
          <Return status="EXTRA">Extra status not in type</Return>
        </StructuredReturns>
      </Agent>
    `;
    expectCompileError(tsx, 'undeclared statuses: EXTRA');
  });
});
```
  </action>
