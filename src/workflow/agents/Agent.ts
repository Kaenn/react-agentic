 ...
 *
 * Extends SpawnAgentProps to accept RuntimeVar values for runtime interpolation
...
export interface V3SpawnAgentProps<TInput = unknown> {
   ...
   * - String: Agent name directly (e.g., 'gsd-researcher')
   * - AgentRef: Type-safe reference from defineAgent()
   * - RuntimeVar: Runtime-resolved agent name
   */
  agent: OrRuntimeVar<string> | AgentRef<TInput>;
  /** Model to use - supports static string or RuntimeVar for runtime resolution */
  model: OrRuntimeVar<string>;
  /** Human-readable description of the task. Accepts static or RuntimeVar. */
  description: OrRuntimeVar<string>;
  /**
   * Enable "load from file" pattern for spawning.
   * Accepts boolean, string path, or RuntimeVar for runtime resolution.
   */
  loadFromFile?: OrRuntimeVar<string | boolean>;
  /** Prompt content - supports multi-line and RuntimeVar interpolation */
  prompt?: OrRuntimeVar<string>;
  /**
   * Typed input - either a RuntimeVar or an object where each value can be static or RuntimeVar.
   * Auto-generates structured prompt from Agent's interface contract.
   */
