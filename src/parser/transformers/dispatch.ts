...
import { Node, JsxElement, JsxSelfClosingElement, JsxFragment, JsxOpeningElement, TemplateExpression } from 'ts-morph';
import type { BlockNode, DocumentNode, BaseBlockNode } from '../../ir/index.js';
import type { TransformContext } from './types.js';
import { isRuntimeContext } from './types.js';
...
import type { XmlBlockNode, GroupNode } from '../../ir/nodes.js';
...
import { transformTable, transformPropList, transformExecutionContext, transformXmlSection, transformXmlWrapper, transformSuccessCriteria, transformOfferNext } from './semantic.js';
import { transformXmlBlock, transformMarkdown, transformCustomComponent } from './markdown.js';
...
import { transformStructuredReturns } from './contract.js';
...
// ============================================================================
// V1 Block Transformation
// ============================================================================

/**
 * Transform a JSX node to BlockNode (V1 path)
...
// ============================================================================
// V1 Children Transformation
// ============================================================================

/**
 * Transform JSX children to BlockNodes (V1 path), handling If/Else and OnStatus sibling pairs
 */
  ...
  while (i < jsxChildren.length) {
    ...
    if (Node.isJsxElement(child) || Node.isJsxSelfClosingElement(child)) {
      ...
      if (childName === 'If' || childName === 'V3If') {
        ...
        const block = transformElement(child, ctx);
        if (block) blocks.push(block);
      }
    ...
    if (expr) {
      ...
      if (Node.isArrowFunction(expr)) {
            ...
            if (Node.isReturnStatement(descendant)) {
              ...
              if (returnExpr) {
                ...
                if (Node.isJsxFragment(inner)) {
                  ...
                  const block = transformElement(inner, ctx);
        ...
        if (Node.isJsxElement(inner) || Node.isJsxSelfClosingElement(inner)) {
          return transformElement(inner, ctx);
        }
      }
...
/**
 * Unified element router — handles both V1 and V3 contexts.
 *
 * V3-only components are guarded behind isRuntimeContext(ctx).
 * Elements that need different V1/V3 handling branch on `runtime`.
 */
function transformElement(
  ...
  const runtime = isRuntimeContext(ctx);

  // ============================================================
  // V3-only components (guarded)
  // ============================================================

  if (runtime) {
    if (name === 'Break') return transformBreak(node, ctx);
    if (name === 'Return') return transformReturn(node, ctx);
    if (name === 'AskUser') return transformAskUser(node, ctx);
    if (isRuntimeFnCall(node, ctx)) return transformRuntimeCall(node, ctx);
    if (name === 'Ref') return transformRef(node, ctx);
    if (name === 'GatherContext') return transformRuntimeGatherContext(node, ctx);
    if (name === 'ComposeContext') return transformRuntimeComposeContext(node, ctx);
    if (name === 'Preamble') return transformRuntimePreamble(node, ctx);
    if (name === 'Indent') return transformRuntimeIndent(node, ctx);
  }

  // ============================================================
  // Branching elements (V1/V3 use different transformers)
  // ============================================================
  ...
  if (headingMatch) {
    ...
    const children = Node.isJsxElement(node)
      ? (runtime ? transformRuntimeInlineChildren(node, ctx) : transformInlineChildren(node, ctx))
      : [];
  ...
  // Code block
  if (name === 'pre') {
    return runtime ? transformRuntimeCodeBlock(node, ctx) : transformCodeBlock(node, ctx);
  }

  // Div
  if (name === 'div') {
    return runtime ? transformRuntimeDiv(node, ctx) as BlockNode : transformDiv(node, ctx);
  }

  // XmlBlock
  if (name === 'XmlBlock') {
    return runtime ? transformRuntimeXmlBlock(node, ctx) : transformXmlBlock(node, ctx);
  }

  // SpawnAgent
  if (name === 'SpawnAgent') {
    return runtime ? transformRuntimeSpawnAgent(node, ctx) : transformSpawnAgent(node, ctx);
  }

  // If / Else
  if (name === 'If' || name === 'V3If') {
    return runtime
      ? transformRuntimeIf(node, ctx, transformRuntimeBlockChildren)
  ...
  if (name === 'Loop' || name === 'V3Loop') {
    return runtime
      ? transformRuntimeLoop(node, ctx, transformRuntimeBlockChildren)
      : transformLoop(node, ctx);
  }

  // List (prop-based)
  if (name === 'List') {
    return runtime ? transformRuntimeList(node, ctx) : transformPropList(node, ctx);
  }
  ...
  // ============================================================
  // Shared elements (same transformer for both)
  // ============================================================

  if (name === 'hr') return { kind: 'thematicBreak' };
  if (name === 'ul') return transformList(node, false, ctx) as BlockNode;
  if (name === 'ol') return transformList(node, true, ctx) as BlockNode;
  if (name === 'blockquote') return transformBlockquote(node, ctx) as BlockNode;
  if (name === 'Table') return transformTable(node, ctx) as BlockNode;
  if (name === 'Assign') return transformAssign(node, ctx);
  if (name === 'AssignGroup') return transformAssignGroup(node, ctx);
  if (name === 'Markdown') return transformMarkdown(node, ctx) as BlockNode;
  if (name === 'SuccessCriteria') return transformSuccessCriteria(node, ctx) as BlockNode;
  if (name === 'OfferNext') return transformOfferNext(node, ctx) as BlockNode;
  if (name === 'XmlSection') return transformXmlSection(node, ctx) as BlockNode;
  if (['DeviationRules', 'CommitRules', 'WaveExecution', 'CheckpointHandling'].includes(name)) {
    return transformXmlWrapper(name, node, ctx) as BlockNode;
  }

  // Swarm components
  if (name === 'TaskDef') return transformTaskDef(node, ctx);
  if (name === 'TaskPipeline') return transformTaskPipeline(node, ctx);
  if (name === 'Team') return transformTeam(node, ctx);
  if (name === 'Teammate') throw ctx.createError('Teammate must be used inside a Team component', node);
  if (name === 'Prompt') throw ctx.createError('Prompt must be used inside a Teammate component', node);
  if (name === 'ShutdownSequence') return transformShutdownSequence(node, ctx);
  if (name === 'Workflow') return transformWorkflow(node, ctx);

  // ============================================================
  // V1-only elements (guarded)
  // ============================================================

  if (!runtime) {
    if (name === 'OnStatus') return transformOnStatus(node, ctx);
    ...
    if (name === 'ReadState') return transformReadState(node, ctx);
    if (name === 'WriteState') return transformWriteState(node, ctx);
    if (name === 'Step') return transformStep(node, ctx);
    if (name === 'Bash') return transformBash(node, ctx);
    if (name === 'ReadFiles') return transformReadFiles(node, ctx);
    if (name === 'PromptTemplate') return transformPromptTemplate(node, ctx);
    if (name === 'Role') {
      const children = Node.isJsxElement(node) ? transformBlockChildren(node.getJsxChildren(), ctx) : [];
    ...
    if (name === 'StructuredReturns') return transformStructuredReturns(node, ctx);
  ...
  if (isCustomComponent(name)) {
    // V3: check local components first
    if (runtime && ctx.localComponents?.has(name)) {
    ...
    // V1: static component composition
    if (!runtime) return transformCustomComponent(name, node, ctx);
  }

  throw ctx.createError(`Unsupported element: <${name}>`, node);
}
