 ...
 * - Step: Numbered workflow steps with heading variant
...
import { Command, SpawnAgent, useOutput, useVariable, Assign, OnStatus, Step, List } from '../../jsx.js';
  ...
  return (
    ...
    >
      ...
      <Step name="Test Objectives" number={1}>
        <List
          items={[
            "Agent A is spawned first and produces initial output",
            "Agent B receives Agent A's output and transforms it",
            "Agent C receives Agent B's output and produces final result",
            "Each agent completes BEFORE the next one spawns",
            "Final result incorporates data from all three agents",
          ]}
          ordered
        />
      </Step>

      <Step name="Workflow Overview" number={2}>
...
└─────────┘    └─────────┘    └─────────┘
 ...
 seedProc      transform     finalResult`}</code></pre>
      ...
      <Step name="Initialize Seed Value" number={3}>
        <p>
      ...
      <Step name="Spawn Agent A (Chain Initiator)" number={4}>
        <p>
...
Return your output in the specified YAML format.`}
        ...
        <OnStatus output={outputA} status="SUCCESS">
          ...
          <p>
            Now proceed to Step 5 to spawn Agent B with Agent A's output.
          </p>
        </OnStatus>
      ...
      <Step name="Spawn Agent B (Chain Transformer)" number={5} variant="bold">
        <p>
...
Return your output in the specified YAML format.`}
        ...
        <OnStatus output={outputB} status="SUCCESS">
          ...
          <p>
            Now proceed to Step 6 to spawn Agent C with Agent B's output.
          </p>
        </OnStatus>
      ...
      <Step name="Spawn Agent C (Chain Finalizer)" number={6} variant="bold">
        <p>
...
Return your output in the specified YAML format.`}
      ...
      <Step name="Validation Report" number={7}>
        <p>
